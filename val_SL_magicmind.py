...
if __name__ == "__main__":
    ...
    network.load_state_dict(torch.load(opt.model_weight_path, map_location="cpu"))

    network.eval()
    
    trace_input = torch.randn(1, 1, 256, 256).float()
    
    model_mm = torch.jit.trace(network, trace_input, check_trace=False)
    torch.jit.save(model_mm, "mm_weight.pt")
    print("Ready Trace\n")
    
    builder = mm.Builder()
    magicmind_network = mm.Network()
    config = mm.BuilderConfig()
    
    parser = Parser(mm.ModelKind.kPytorch)
    parser.set_model_param("pytorch-inputs-dtypes", [mm.DataType.FLOAT32])
    parser.parse(magicmind_network, "mm_weight.pt")
    ...
    batch_size = 1
    assert magicmind_network.get_input(0).set_dimension(mm.Dims((batch_size, 1, 256, 256))).ok()
    
    assert config.parse_from_string('{"convert_input_layout": {"0": {"src": "NCHW", "dst": "NCWC"}}}').ok()
    
    assert config.parse_from_string('{"insert_bn_before_firstnode": {"0": {"mean": [0, 0, 0], "var": [65025, 65025, 65025]}}}').ok()
    
    model = builder.build_model("my_mm_model", magicmind_network, config)
    assert model != None
    
    model.serialize_to_file("/mm_model")
    print("Generate Model Done, model save to", "/mm_model")
    
    with mm.System() as sys:
        dev = mm.Device()
        dev.id = 0
        assert dev.activate.ok()
        ...
        null_tensor_1 = torch.zeros([1,1,256,128])
        null_tensor_2 = torch.zeros([1,1,128,256])
        null_tensor_3 = torch.zeros([1,1,128,128])
        ...
        for valid_name, valid_images in valid_dict.items():  # 1次循环
            ...
            for k in range(repeat_times):  # 循环10 3 20次
                ...
                for idx, im in enumerate(valid_images):
                    ...
                    noisy255 = np.clip(noisy255 * 255.0 + 0.5, 0, 255).astype(np.uint8)
                    ...
                    image = np.zeros((w, h, 1), dtype=np.float32)
                    image[w_c: w_c + W, h_c: h_c + H, 0] = img
                    prediction = np.zeros((w, h), dtype=np.float32)
                    with torch.no_grad():
                        for i in range(0, w, step):
                            for j in range(0, h, step):
                                ...
                                # new_image = torch.zeros([1,1,256,256])
                                outputs = []
                                # UNNORMAL!
                                ...
                                if in_patch.shape == (1,1,256,128):
                                    ...
                                    # new_image = compiled_model(in_patch)
                                    assert context.enqueue(in_patch, outputs, queue).ok()
                                    assert queue.sync().ok()
                                    new_image = torch.from_numpy(np.array(outputs[0].asnumpy()))
